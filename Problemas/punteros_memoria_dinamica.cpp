...
bool resuelveCaso() {
    ...
    if (numImagenes == 0)
        return false;
    std::cin.ignore(256, '\n');
    std::vector <tFoto*> vpFotos(numImagenes);
    for (int i = 0; i < numImagenes; i++) {
        vpFotos[i] = new tFoto;
        (vpFotos[i]->foto).resize(50, std::vector<int>(50, i));
        std::getline(std::cin, vpFotos[i]->titulo);

    }
    for (int i = 0; i < numImagenes; i++) {
        std::getline(std::cin, vpFotos[i]->tema);
    }
    std::vector <tFoto*> v1 = vpFotos;
    resolver(vpFotos, v1);

    // escribir sol
    for (int i = 0; i < numImagenes; i++) {
        std::cout << vpFotos[i]->titulo << '\n';
    }
    std::cout << '\n';
    for (int i = 0; i < numImagenes; i++) {
        std::cout << v1[i]->tema << " - " << v1[i]->titulo << '\n';
    }
    std::cout << '\n';
    for (int i = 0; i < numImagenes; i++) {
        delete vpFotos[i];
