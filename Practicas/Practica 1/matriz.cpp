...
bool cargar(tMatrizChar& mat, std::ifstream& flujo) { //La función devolverá true si se logra cargar la matriz acorde a las dimensiones establecidas
...
bool rango(tMatrizChar mat, tCoor pos1, tCoor pos2) {   //Utilizada para comprobar que un par de coordenadas son válidas de acuerdo a la matriz
...
bool swapD(tMatrizChar& mat, int d) {
    ...
    if (mat.size() == mat[0].size()) {  //Necesitamos que la matriz sea cuadrada para aplicar la operación
        if (diagonal < 0)
            diagonal = 0 - diagonal;    //Intercambiar una diagonal positiva o negativa es lo mismo, así que trabajamos con la positiva por comodidad
        coor1.fila = 0;
        coor1.col = diagonal;
        coor2.fila = diagonal;
        coor2.col = 0;
